...
'i':{'name': 'index', 'unit': 'count'},
'j':{'name': 'indexJ', 'unit': 'count'},
'k':{'name': 'indexK', 'unit': 'count'},
}
...
axislib=axislib0):
  ...
  if not newfile1:
    #temp1 = string.join(list(nc.dimensions.keys()), ' ')
    temp1 = ' '.join(list(nc.dimensions.keys()))
    # not to use 'd'
    temp3 = string.ascii_letters[:4] + string.ascii_letters[5:26]
    for i in temp3:
  ...
  for i in range(len(array.shape)):
    # get axis name
    if axisorder1[i] in list(axislib.keys()):
    ...
    existing1 = 0
    if not newfile1:
      if axisName1[i] in list(nc.dimensions.keys()):
  ...
  if name:
    ...
    if name1 in list(nc.variables.keys()):
    ...
    name1 = 'data'
    i = 0
    while name1 in list(nc.variables.keys()):
      i += 1
      name1 = 'data%d'%i
...
'''
  ...
  for o in opList:
    if o['varName']:
      if o['varName'] not in list(nc.variables.keys()):
        nc.close()
  ...
  # create_file_attributes
  for a in list(nc.__dict__.keys()):
  ...
  for v in list(nc.dimensions.keys()):
    dimName9[v] = v
  ...
  for o in opList:
    ...
    if o['varExp']:
      ...
      # change_dimension_name
      if o['varName'] in list(nc.dimensions.keys()):
        dimName9[o['varName']] = o['varNameNew']

  # inverse_varName9, to be used in att setting
  varName10 = {}
  for v in list(varName9.keys()):
    varName10[varName9[v]] = v 

  # create_dimensions
  for d in list(nc.dimensions.keys()):
  ...
  for v in list(nc.variables.keys()):
    ...
    if len(dim8)==1:
      ...
      print('assign variable: ', varValue9[v], '  :', end=' ')
    ...
    # assign_attributes
    for a in list(var3.__dict__.keys()):
